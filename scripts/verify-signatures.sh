...
if [ -f "ipcprims-minisign.pub" ]; then
	for manifest in SHA256SUMS SHA512SUMS; do
		if [ -f "$manifest" ] && [ -f "${manifest}.minisig" ]; then
			...
			if minisign -Vm "$manifest" -p ipcprims-minisign.pub; then
	...
	echo "[!!] ipcprims-minisign.pub not found - cannot verify minisign signatures"
	ERRORS=$((ERRORS + 1))
fi
...
if [ -f "ipcprims-release-signing-key.asc" ]; then
	...
	gpg --import ipcprims-release-signing-key.asc 2>/dev/null
