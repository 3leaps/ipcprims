...
#[cfg(unix)]
fn unique_ipc_path(tag: &str) -> PathBuf {
...
#[cfg(unix)]
fn nonexistent_ipc_path() -> PathBuf {
    PathBuf::from(format!(
...
fn info_against_echo_server_outputs_connection_data() {
    let sock_path = unique_ipc_path("info");
    ...
    wait_for_connect(&sock_path, &[1], Duration::from_secs(5));
...
fn info_timeout_returns_124() {
    let missing = nonexistent_ipc_path();
